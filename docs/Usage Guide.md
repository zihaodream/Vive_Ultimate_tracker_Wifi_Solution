...
1. UTK is recommended to use system version 169. This project was developed and
   tested on that version. Other versions may have unknown issues; versions 168
   and 169 have been tested as usable, while version 178 has not been tested
   yet.
...
Shows the number of connected trackers and their corresponding IP addresses.
`Power off selected` powers off the selected device, and `Power off all` powers
off all connected devices.

`Dongle Mode`

Switches the device back to official receiver mode(using Vive Hub). This is usually used when
...
### What if the device cannot connect to the PC after Flash WiFi?

1. Check whether the WiFi name or password is wrong. If there is an error,
   enter the correct information and run `Flash WiFi` again.
2. If the WiFi name and password are correct, the write process may have failed.
   Use `Dongle Mode` to clear the WiFi-only configuration and switch back to
   official receiver mode.
3. If the device still cannot recover, use the `APQ8053_ROM_FB.zip` firmware
   package to restore the system.

### What if I need to update or rescan the map?

Use `Dongle Mode` to switch back to official receiver mode, then update the map
through VIVE Hub. After the map update is complete, use `Flash WiFi` again to
write the device back to WiFi-only mode.
